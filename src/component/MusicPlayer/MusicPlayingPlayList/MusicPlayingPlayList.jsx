...
function MusicPlayingPlayList() {
  ...
  const playingPlaylist = useSelector(
    (state) => state.musicPlayer.playingPlaylist
  );
  ...
  function findMusic(e, selector) {
    const music = playingPlaylist.find(
      (item) => item.key === findElement(e, selector)
    );
    return music;
  }
    ...
    <ul className="music-player-playlist">
      ...
      {playingPlaylist.map((item, index) => {
        if (item.key !== playingSong.key) {
          return (
            <li
              className="music-player-playlist__playing"
              key={index}
              data-key={item.key}
              onClick={(e) => {
                dispatch(
                  setPlayingSong(
                    findMusic(e, ".music-player-playlist__playing")
                  )
                );
              }}
            >
              ...
              <div className="music-player-playlist__playing__options">
                <i
                  className="fas fa-trash-alt music-player-playlist__playing__options__item"
                  onClick={(e) => {
                    dispatch(
                      slicePlayingPlaylist(
                        findMusic(e, ".music-player-playlist__playing")
