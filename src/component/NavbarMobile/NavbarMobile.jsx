...
function NavbarMobile() {
  ...
  const active = headerNav.findIndex(
    (e) => e.path.split("/explore")[0] === pathname.split("/explore")[0]
  );
    ...
    <div className="navbar-mobile-container">
      ...
      <ul className={`navbar-menu ${showMenu && "--active-menu"}`}>
        <li className="navbar-menu__item">
          ...
          {login && <InforUser data={{ src: picture, name: data.name }} />}
        </li>
        {headerNav.map((e, i) => {
          return (
            <li
            ...
            >
              <Link
                className={`navbar-menu__item__link representation ${
                  i === active ? "active" : ""
                } `}
                to={e.path}
                key={i}
              >
                ...
                <span className="navbar-menu__item__title">{e.display}</span>
              </Link>
            </li>
          );
        })}
